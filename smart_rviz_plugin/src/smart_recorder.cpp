...
namespace smart_rviz_plugin
{
    ...
    void SmartRadarRecorder::initializeRecorder()
    {
        ...
        table_data_->setHorizontalHeaderLabels({"RadialSpeed [m/s]", "Power [dB]", "RCS [dB]", "Noise [dB]", "SNR", "AzimuthAngle [rad]", "ElevationAngle [rad]", "Range [m]"});
    ...
    void SmartRadarRecorder::callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg, const std::string topic_name)
    {
        if (!selected_topic_.empty() && topic_name == selected_topic_)
        {
            ...
            for (size_t i = 0; i < msg->height * msg->width; ++i, ++iter_radial_speed, ++iter_power, ++iter_rcs, ++iter_noise, ++iter_snr, ++iter_azimuth_angle, ++iter_elevation_angle, ++iter_range)
            {
                ...
                table_data_->setItem(0, 5, new QTableWidgetItem(QString::number(*iter_azimuth_angle, 'f', 2)));
                table_data_->setItem(0, 6, new QTableWidgetItem(QString::number(*iter_elevation_angle, 'f', 2)));
            ...
            {
                recorded_data.append({radial_speed_value_str, power_value_str, rcs_value_str, noise_value_str, snr_value_str, azimuth_value_str, elevation_value_str, range_value_str});
    ...
    void SmartRadarRecorder::saveDataToCSV()
    {
        ...
        if (!recorded_data.isEmpty())
        {
            ...
            QString file_path = file_dialog.getSaveFileName(this, "Save Data", "", "CSV Files (*.csv);;All Files (*)");
            ...
            if (!file_path.isEmpty())
            {
                ...
                if (csvfile.open(QIODevice::WriteOnly | QIODevice::Text))
                {
                    ...
                    csv_writer << "RadialSpeed [m/s], Power [dB], RCS [dB], Noise [dB], SNR, AzimuthAngle [rad], ElevationAngle [rad], Range [m], TimestampSec, TimestampNanoSec \n";
