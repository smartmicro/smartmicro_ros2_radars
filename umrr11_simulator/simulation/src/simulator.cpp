...
{   
    ...
    for (auto instruction: instructions)
        ...
        {   
            std::cout << "Base user interface major version for UMRR11 set!"  << std::endl;
        ...
        {   
            std::cout << "Base user interface minor version for UMRR11 set!"  << std::endl;
            instruction->SetResponse(COM_INSTR_PORT_SUCCESS);
            instruction->SetValue(0);    
        }
        else if(instruction->GetSectionId() == 3042 && instruction->GetId() == 22)
        {   
            std::cout << "User interface identifier for UMRR11 set!"  << std::endl;
            instruction->SetResponse(COM_INSTR_PORT_SUCCESS);
            instruction->SetValue(4);
        }
        else if(instruction->GetSectionId() == 3042 && instruction->GetId() == 23)
        {   
            std::cout << "User interface major version for UMRR11 set!"  << std::endl;
            instruction->SetResponse(COM_INSTR_PORT_SUCCESS);
            instruction->SetValue(1);
        }
        else if(instruction->GetSectionId() == 3042 && instruction->GetId() == 24)
        {   
            std::cout << "User interface minor version for UMRR11 set!"  << std::endl;
            instruction->SetResponse(COM_INSTR_PORT_SUCCESS);
            instruction->SetValue(1);
        }
        else
...
int main()
{
    ...
    {   
        ...
        std::string portFile = "/code/umrr11_simulator/port_radar_targets.bin";
        ...
        std::cout << "sensor UMRR11 is transmitting data! " << std::endl;
        ifs.close();
        delete[] filebuffer;
