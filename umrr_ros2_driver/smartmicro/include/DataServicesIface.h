...
typedef std::function<void(ClientId clientId, PortId, BufferDescriptor)>
...
typedef std::function<void(ClientId id, bool clientStatus)>
    ConnectedInfoCallback;
...
public:
   ...
   *            IN const InstructionBuffer& instBuffer - instruction port
   * object. IN ResponseCallback callback - callback function which should be
   *                                           called upon a response.
   *
   * Return: ErrorCode
   * Description: This function sends an instruction port to the input client.
   * It is used to send requests and responses. In the case of a response, no
   ...
   * this function will pass all deserialzied data ports to centrally registered
   * callback function
   ...
   */
  virtual ErrorCode
  RegisterDataRecvCallback(IN DataReceiverCallback callback) = 0;

  /*
   * Function: RegisterDataRecvCallback
   * Arguments: IN ClientId clientId - client id of the sending client
   *            IN PortId portId - port id of the required port
   *            IN DataReceiverCallback callback - callback function which
   * should be called upon a reception. Return: ErrorCode Description: Registers
   * a receiver callback function , which should be called , when a new buffer
   * arrives.
   */
   ...
   * Arguments: IN DataReceiverCallback callback - callback function will be
   called upon a reception of new instruction port.
   * Return: ErrorCode
   * Description: Registers a receiver callback function, which should be
   called, when new instructions arrive.
   */
  virtual ErrorCode
  RegisterInstRecvCallback(IN DataReceiverCallback callback) = 0;
   ...
   */
  virtual ErrorCode StreamDataPort(IN ClientId clientId, IN PortId portId,
   ...
   */
  virtual ErrorCode StreamInternalDataPort(ClientId clientId, PortId portId,
   ...
   */
  virtual ErrorCode
  RegisterConnectedClientCallback(IN ConnectedInfoCallback callback) = 0;

  /*
   * Function:    RegisterGetLocalTimeCallback
   * Arguments:   GetLocalTimeCallback
   * Return:      ErrorCode
   * Description: Register a callback to receive the local time for the time
   * sync feature. Valid for time sync master and slave
   */
  virtual ErrorCode
  RegisterGetTimeCallback(IN GetLocalTimeCallback callback) = 0;

  /*
   * Function: RegisterSetTimeOffsetCallback
   * Arguments: SetTimeOffsetCallback
   * Return: ErrorCode
   * Description: Register a callback to set adjust the local time with the
   * offset Valid for time sync slave.
   */
  virtual ErrorCode
  ...
  /*
   * Function: Configure Time Sync Slave. an only be configured if the service
   *           is not running.
   * Arguments: uint8_t Number of required Iteration to synchronize timestamp.
   * Return: ErrorCode
   * Description: Valid only for time sync slave and sets the required number of
   * iteration.
   */
  virtual ErrorCode ConfigureTimeSyncSlave(uint8_t requireIteration) = 0;

  /*
   * Function: Configure Time Sync Master. Can only be configured if the service
   *           is not running.
   * Arguments: uint8_t Cooldown time between two time sync
   * intervals. uint8_t Maximum number of iteration which support time sync
   *                    master service. It is not possible to synchronize
   *                    sensors that require a higher iteration number than the
   *                    maximum number of iterations.
   * Return: ErrorCode Description: Valid only for
   * time sync master and sets cooldown time between two time sync intervals.
   */
  virtual ErrorCode ConfigureTimeSyncMaster(uint8_t cooldownTime,
                                            uint8_t maxNumberOfIteration) = 0;
   ...
   */
  virtual ErrorCode
  StartSWUpdate(IN std::map<uint32_t, SWUpdateConfigData> &updateList,
                IN ClientId clientID, IN OnUpdateInfoCallback callback) = 0;
  /*
  ...
  /*
   * Function: GetConfig
   * Arguments: OUT ComLibConfig
   * Return: bool
   * Description: Get data from ComLibConfig file
   */
  virtual bool GetConfig(OUT ComLibConfig &config) = 0;
  /*
   * Function: GetTimeSyncMasterTimeOutInfo
   * Return: const TimeOutMap as reference
   * Description: Gets the time-out details from TimeSync master
   */
  virtual TimeOutMap GetTimeSyncMasterTimeOutInfo() = 0;
};
